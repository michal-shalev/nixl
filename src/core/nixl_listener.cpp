...
public:
    nixlEtcdClient(const std::string &my_agent_name, std::chrono::seconds timeout_sec = 5)
    ...
    nixl_status_t waitForMetadataFromEtcd(const std::string& metadata_key,
        ...
        try {
            ...
            auto status = future.wait_for(std::chrono::seconds(watch_timeout_sec));
