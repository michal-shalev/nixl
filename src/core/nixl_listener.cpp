...
private:
    ...
    std::chrono::seconds watch_timeout_sec;
...
public:
    nixlEtcdClient(const std::string &my_agent_name, std::chrono::seconds timeout_sec = std::chrono::seconds(5))
    ...
    nixl_status_t waitForMetadataFromEtcd(const std::string& metadata_key,
        ...
        try {
            ...
            auto status = future.wait_for(watch_timeout_sec);
...
void nixlAgentData::commWorker(nixlAgent* myAgent){
    ...
    if(useEtcd) {
        etcdClient = std::make_unique<nixlEtcdClient>(name, config.etcdWatchTimeoutSec.count());
    }
#endif // HAVE_ETCD
